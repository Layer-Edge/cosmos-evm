...
if [[ $overwrite == "y" || $overwrite == "Y" ]]; then
	...
	jq '.app_state["bank"]["denom_metadata"]=[{"description":"The native staking token for evmd.","denom_units":[{"denom":"edgens","exponent":0,"aliases":["edgens"]},{"denom":"edgen","exponent":18,"aliases":[]}],"base":"edgens","display":"edgen","name":"LayerEdge Testnet Staking Token","symbol":"EDGE","uri":"","uri_hash":""}]' "$GENESIS" >"$TMP_GENESIS" && mv "$TMP_GENESIS" "$GENESIS"
	...
	sed -i.bak 's/minimum-gas-prices = "0aatom"/minimum-gas-prices = "0.025edgens"/g' "$APP_TOML"
