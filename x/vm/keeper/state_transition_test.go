...
func (suite *KeeperTestSuite) TestApplyTransaction() {
	...
	err := suite.network.App.BankKeeper.MintCoins(ctx, "mint", sdk.NewCoins(sdk.NewCoin("aedgen", sdkmath.NewInt(3e18))))
	suite.Require().NoError(err)
	err = suite.network.App.BankKeeper.SendCoinsFromModuleToModule(ctx, "mint", "fee_collector", sdk.NewCoins(sdk.NewCoin("aedgen", sdkmath.NewInt(3e18))))
	suite.Require().NoError(err)
	testCases := []struct {
		name       string
		gasLimit   uint64
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.name), func() {
			...
			suite.Require().NoError(err)
			initialBalance := suite.network.App.BankKeeper.GetBalance(ctx, suite.keyring.GetAccAddr(0), "aedgen")
			...
			balanceAfterRefund := suite.network.App.BankKeeper.GetBalance(ctx, suite.keyring.GetAccAddr(0), "aedgen")
