...
edge network that consists of at least one or potentially many validators. This
edge network can be used primarily for integration tests or unit edge suites.

The edge network utilizes SimApp as the ABCI application and uses all the modules
defined in the Cosmos SDK. An in-process edge network can be configured with any
number of validators as well as account funds and even custom genesis state.

When creating a edge network, a series of Validator objects are returned. Each
...
exposed. Due to this exact same limitation, only a single edge network can exist
