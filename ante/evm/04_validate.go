...
func CheckTxFee(txFeeInfo *tx.Fee, txFee *big.Int, txGasLimit uint64) error {
	...
	// to MsgEthereumTx, which is a sdk tx. Here, the denom will be a uatom, not aedgen.
	// BuildTx then converts uatom to aedgen meaning that logic that interacts with the user
	// will use uatom and internal processing such as the ante handler will operate based on aedgen.
