	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			if tc.expPass {
				...
				s.Require().Equal([]cmn.Coin{{Denom: "aedgen", Amount: big.NewInt(100)}}, out.Deposit.Amount)
			} else {
				s.Require().Error(err)
	...
	testCases := []struct {
		...
		},
		{
			...
			func(data *gov.ProposalData) {
				...
				s.Require().Equal("edge prop", data.Title)
				s.Require().Equal("edge prop", data.Summary)
	...
	testCases := []struct {
		...
		},
		{
			...
			func(data []gov.ProposalData, pageRes *query.PageResponse) {
				...
				s.Require().Equal("edge prop", proposal.Title)
				s.Require().Equal("edge prop", proposal.Summary)
